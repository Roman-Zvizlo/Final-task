class Cart {
  ...
  order(ev) {
    if (this.cartLength() === 0) {
      window.showAlert('Будь ласка оберіть товар для замовлення', false);
      return;
    }    
      ...
      fetch('order', {
        ...
        .then(response => {
          if (response.status === 200) {
            ...
            throw new Error('Неможливо відправити форму');
          }
        })
        .then(responseText => {
          form.reset();
        ...
        .catch(error => showAlert('Сталась помилка: ' + error, false));
    } else {
